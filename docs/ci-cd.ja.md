...
| `-w, --workflow <name or path>` | Workflow 名または workflow YAML ファイルのパス（`--piece` は互換 alias として引き続き利用可能） |
...
| `--provider <name>` | エージェント provider を上書き（claude\|codex\|opencode\|cursor\|copilot\|gemini\|mock） |
